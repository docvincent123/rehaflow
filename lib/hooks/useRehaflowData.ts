...
async function fetchWebPatients(): Promise<Patient[]> {
  const payload = await apiRequest('/patients/active', { client: 'web', query: { status: 'active', limit: 500 } });
  ...
  }).filter((patient) => patient.id).filter((patient) => {
...
async function fetchActivePatientCount(): Promise<number> {
  const payload = await apiRequest('/patients/active', { client: 'web', query: { status: 'active', limit: 1, offset: 0 } });
  const record = asRecord(payload);
  const explicit = Number(record.total ?? record.count ?? record.totalPatients ?? record.activeCount);
  if (Number.isFinite(explicit) && explicit >= 0) return explicit;
  return pickCollection(payload, ['patients', 'activePatients', 'data', 'items', 'rows', 'records']).length;
}

export function usePatientsQuery() {
  const online = useIsOnline();
  const queryClient = useQueryClient();
  const patientsQuery = useQuery({ queryKey: queryKeys.patients, queryFn: fetchWebPatients, staleTime: Infinity, gcTime: 1000 * 60 * 60 * 24, refetchOnMount: false, refetchOnWindowFocus: false, refetchOnReconnect: false });
  const versionQuery = useQuery({ queryKey: ['patients', 'version'], queryFn: fetchActivePatientCount, enabled: online, staleTime: 8_000, refetchInterval: 8_000, refetchIntervalInBackground: false, refetchOnWindowFocus: false });

  useEffect(() => {
    const count = versionQuery.data;
    if (typeof count !== 'number') return;
    const current = queryClient.getQueryData<Patient[]>(queryKeys.patients) ?? [];
    if (count > current.length) void queryClient.invalidateQueries({ queryKey: queryKeys.patients, exact: true });
...
export function useTasksQuery() { const online = useIsOnline(); return useQuery({ queryKey: queryKeys.tasks, queryFn: fetchTasks, refetchInterval: online ? POLL_INTERVAL_MS : false, refetchIntervalInBackground: false, staleTime: 5_000 }); }
export function useTaskQuery(id: string | undefined) { const online = useIsOnline(); return useQuery({ queryKey: queryKeys.task(id ?? ''), queryFn: () => fetchTask(id ?? ''), enabled: Boolean(id), refetchInterval: online ? POLL_INTERVAL_MS : false, refetchIntervalInBackground: false }); }
