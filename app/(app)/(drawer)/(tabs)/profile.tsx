    ...
    <View className="bg-background flex-1">
      ...
      <ScrollView contentContainerClassName="gap-4 px-4 pb-10 pt-4">
        <View className="border-border bg-surface flex-row items-center gap-3 rounded-2xl border p-4">
          <View className="bg-accent h-14 w-14 items-center justify-center rounded-full"><Text className="text-accent-foreground text-lg font-bold">{initials(user?.fullName ?? '—')}</Text></View>
        ...
        {canTrackShift(user?.role ?? null) ? (
          <SectionCard title="Зміна">
            ...
            <Pressable disabled={pending !== null} onPress={() => void (shift?.isActive ? finish() : begin())} className={`mt-3 min-h-12 items-center justify-center rounded-xl px-4 py-3 ${shift?.isActive ? 'border-danger bg-danger/15 border' : 'bg-accent'}`}>
              <Text className={`text-sm font-bold ${shift?.isActive ? 'text-danger' : 'text-accent-foreground'}`}>{shift?.isActive ? 'Завершити зміну' : 'Почати зміну'}</Text>
            </Pressable>
          </SectionCard>
        ) : null}
        ...
        <Pressable onPress={() => router.push('/devices')} className="border-border bg-surface min-h-14 flex-row items-center gap-3 rounded-2xl border px-4 py-3.5"><Smartphone color={navColors.accent} size={20} /><Text className="text-foreground flex-1 text-[15px] font-medium">Підключені пристрої</Text><Text className="text-accent text-sm font-semibold">{activeDevices.length}</Text></Pressable>
        {!confirm ? (
          <Pressable onPress={() => setConfirm(true)} className="bg-danger/15 min-h-14 flex-row items-center justify-center gap-2 rounded-xl px-5 py-4"><LogOut color={navColors.danger} size={18} /><Text className="text-danger text-base font-bold">Вихід</Text></Pressable>
        ) : (
          <View className="border-danger/40 bg-danger/10 gap-2 rounded-2xl border p-4">
            ...
            <View className="flex-row gap-2">
              <Pressable onPress={() => void signOut()} className="bg-danger min-h-12 flex-1 items-center justify-center rounded-xl px-4 py-3"><Text className="text-danger-foreground text-sm font-bold">Так, вийти</Text></Pressable>
