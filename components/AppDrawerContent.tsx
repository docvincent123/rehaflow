...
import { Clock, LogOut, Smartphone, SquarePlay, SquareStop } from 'lucide-react-native';
        ...
        <View className="gap-4 px-4 pt-4">
          ...
          {canTrackShift(user?.role ?? null) ? (
            <View className="border-border bg-surface gap-3 rounded-2xl border p-4">
              <View className="flex-row items-center gap-2"><Clock color={navColors.muted} size={16} /><Text className="text-muted text-xs font-bold tracking-wide uppercase">Зміна</Text></View>
              ...
              {shift?.isActive ? (
                ...
                <Pressable disabled={pending !== null} onPress={() => void begin()} className="bg-accent min-h-12 flex-row items-center justify-center gap-2 rounded-xl px-4 py-3"><SquarePlay color={navColors.headerForeground} size={16} /><Text className="text-accent-foreground text-sm font-semibold">Почати зміну</Text></Pressable>
              )}
            </View>
          ) : null}

          <Pressable onPress={() => go('/devices')} className="border-border bg-surface min-h-12 flex-row items-center gap-3 rounded-xl border px-4 py-3"><Smartphone color={navColors.muted} size={18} /><Text className="text-foreground text-sm font-medium">Підключені пристрої</Text></Pressable>
        </View>
