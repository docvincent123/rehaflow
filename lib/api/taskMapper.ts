...
import { mapPriority, mapTaskStatus, mapPrescriptionType } from './mappers';
...
function targetRoleOf(record: Record<string, unknown>): TaskTargetRole {
  ...
  return raw && /(rehab|реаб|physio|фізіо|кінез)/i.test(raw) ? 'REHAB_SPECIALIST' : 'NURSE';
}
...
export function mapTask(input: unknown): MedicalTask {
  ...
  const targetRole = targetRoleOf(record);
  ...
  return {
    ...
    typeLabel: readString(record, ['typeLabel', 'categoryLabel']) ?? (typeRaw ? typeRaw : undefined),
