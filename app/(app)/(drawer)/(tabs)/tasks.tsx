...
import { configureTaskAlerts, ringForNewTask, startTaskRinging, stopTaskRinging } from '@/lib/notifications/taskAlerts';
...
function buildRows(tasks: MedicalTask[], splitUrgent: boolean): Row[] {
  ...
  const rows: Row[] = [];
  if (urgent.length) { rows.push({ kind: 'header', id: 'urgent', title: `Термінові (${urgent.length})` }); urgent.forEach((task) => rows.push({ kind: 'task', id: task.id, task })); }
  if (normal.length) { rows.push({ kind: 'header', id: 'normal', title: `Звичайні (${normal.length})` }); normal.forEach((task) => rows.push({ kind: 'task', id: task.id, task })); }
  return rows;
}
...
export default function TasksScreen() {
  ...
  useEffect(() => {
    ...
    if (!firstLoad && fresh.length > 0) {
      ...
      setAlertMessage(newest.priority === 'URGENT' ? `Термінове: ${newest.patientName} · ${newest.title}` : `Нове завдання: ${newest.patientName} · ${newest.title}`);
  ...
  const pendingTaskIds = useMemo(() => {
    ...
    for (const action of queuedActions) if (action.kind !== 'CREATE_PRESCRIPTION') ids.add(action.taskId);
    return ids;
  }, [queuedActions]);
